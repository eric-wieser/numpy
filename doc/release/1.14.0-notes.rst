...
repr(a.item()) + ')'`` respectively for 0d array ``a``. This is unlike both
numpy scalars and higher dimension ndarrays.

Instead, the ``str`` of a 0d array now prints like a numpy scalar as
...
(the new default), which causes 0d arrays to be printed using the specified
